...
- **`vault/`** — pure crypto + on-disk format. AES-256-GCM with Argon2id KDF. The header parameters must always match the derivation parameters of the encryption key. Session-key saves preserve those parameters; weak parameters are ratcheted only during password unlock. `Encrypt`/`Decrypt` know nothing about prompts or files; `LoadFile`/`SaveFile`/`SaveSession` are the I/O layer.
...
- **`ssh/`** — wraps `golang.org/x/crypto/ssh`. `Dial` supports one saved jump host. `Interactive(c, opts)` opens a PTY session with optional output teeing; `Run(ctx, c, cmd)` is the cancellable non-interactive path used by `exec`. Native connections implement keepalives and port forwarding, but X11 and arbitrary OpenSSH options are export-only. **Never shell out to `/usr/bin/ssh`** from this package.
